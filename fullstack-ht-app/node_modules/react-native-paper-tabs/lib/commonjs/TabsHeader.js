...
function TabsHeader({
  ...
  let isDark;
  const backgroundColor = customBackground ? customBackground : isDarkTheme && themeMode === 'adaptive' ? (0, _reactNativePaper.overlay)(elevation, colors.surface) : colors.primary;
  ...
  const [indicatorRef, updateIndicator, indicatorStyle] = (0, _internal.useIndicator)({
    tabsLayout,
    layouts,
    index,
    offset,
  ...
  React.useEffect(() => {
    updateScroll();
  }, [updateScroll]); // update indicator when index changes (updateIndicator function changes to new reference when index changes)
  ...
  }, /*#__PURE__*/React.createElement(_reactNativePaper.Surface, {
    style: [{
      backgroundColor,
      elevation
